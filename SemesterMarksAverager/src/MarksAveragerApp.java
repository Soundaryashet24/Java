...
public class MarksAveragerApp {
public static void main(String[] args) {
	Scanner scan = new Scanner(System.in);
	System.out.println("Enter semester marks respectively:");
	...
	scan.close();// if we have static method in different class and we need to access it then we have to call it by using it's class name.method call
}
public static double calculateAverage(int sem1, int sem2, int sem3, int sem4, int sem5, int sem6, int sem7, int sem8) {
	return (sem1+sem2+sem3+sem4+sem5+sem6+sem7+sem8)/8.0;//int/float = float so 8.0 else value will be in int
}
}
